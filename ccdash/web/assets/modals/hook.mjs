...
import { modalBox, numCell, originCols, renderTable } from "../components.mjs";

const hookDetail = (d) =>
  modalBox({
    title: `Hook: ${escapeHtml(d.name)}`,
    cap: `${escapeHtml(d.event || "")}${
      d.regs?.length
        ? " &middot; " +
          d.regs
            .map((row) =>
              escapeHtml([row.source, row.type, row.matcher].filter(Boolean).join(" / ")),
            )
            .join(", ")
        : ""
    }${
